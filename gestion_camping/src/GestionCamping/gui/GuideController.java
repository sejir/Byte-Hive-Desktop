  ...
  GuideCRUD ge =new GuideCRUD();
    ...
    private void supprimguide(ActionEvent event) {
        GuideCRUD ge = new GuideCRUD();
         ...
         ge.SupprimerGuide(g);
         int selectedID = tguide.getSelectionModel().getSelectedIndex();
         tguide.getItems().remove(selectedID);
        afficherguide();
        Stage stage;
    ...
    private void impdf(ActionEvent event) {
         try { 
                ...
                PdfWriter.getInstance(doc, new FileOutputStream("C:\\Users\\USER\\Desktop\\guide.pdf"));
                doc.open();
...
//                 table.addCell("2.3");
                ...
                doc.close();
                Desktop.getDesktop().open(new File("C:\\Users\\USER\\Desktop\\guide.pdf"));
             } catch (Exception e) {
          System.err.print(e);
        }
    }
...
System.out.println("Success...");
    ...
    private void afficherg(ActionEvent event) {
        ...
        glieux.setCellValueFactory(new PropertyValueFactory<>("lieux"));
        gdispo.setCellValueFactory(new PropertyValueFactory<>("disponoibilte"));
            GuideCRUD up = new GuideCRUD();
        List<Guide> liste = up.listerGuide();
        ObservableList<Guide> olist = FXCollections.observableArrayList(liste);
        
        tguide.setItems(olist);
    }
    }
